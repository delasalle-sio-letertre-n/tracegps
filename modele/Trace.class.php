...
class Trace
{
    ...
    public function getCentre() {
        if ($this->getNombrePoints() > 0){
            ...
            for ($i = 0; $i < $this->getNombrePoints() - 1; $i++)
            {
                ...
                if ($latmini > $ptlat) $latmini = $ptlat;
                if ($latmaxi < $ptlat) $latmaxi = $ptlat;
                if ($lonmini > $ptlon) $lonmini = $ptlon;
                if ($lonmaxi < $ptlon) $lonmaxi = $ptlon;
            }
    ...
    public function getDenivele(){
        if ($this->getNombrePoints() > 0)
        {
            ...
            for ($i = 0; $i < $this->getNombrePoints() - 1; $i++)
            {
                ...
                if ($altmini > $ptalt) $altmini = $ptalt;
                if ($altmaxi < $ptalt) $altmaxi = $ptalt;
            }
    ...
    public function getDureeTotale() {
        ...
        $res = "";
        if($heures < 10){$res =$res . "0" . $heures;}
        else{$res = $res.$heures;}
        $res =$res . ":";
        if($minutes < 10){$res =$res . "0" . $minutes;}
        else {$res = $res . $minutes;}
        $res =$res . ":";
        if($secondes < 10){$res =$res . "0" . $secondes;}
        else{$res =$res . $secondes;}
        return $res;
    ...
    {
        if ($this->getNombrePoints() > 0) return $this->lesPointsDeTrace[$this->getNombrePoints() - 1]->getDistanceCumulee();
        return 0;
    }
